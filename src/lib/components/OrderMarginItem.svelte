...
{#each order.Details as detail}
  <div class="order">
    ...
    <div class="detail_item"><div>値段</div><div>{detail.Price}</div></div>
    <div class="detail_item">{OrderTypeEnum[detail.OrdType]}</div>
    <div class="detail_item"><div>受渡日</div><div>{formatDate(detail.DelivDay)}</div></div>
    <div class="detail_item"><div>手数料</div><div>{detail.Commission}</div></div>
  </div>
{/each}
